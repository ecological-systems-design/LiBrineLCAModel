...
def proc_SiFe_removal_limestone(m_in=m_out) :
    ...
    df_data = {
        'Variables' : [f'm_output_{process_name}', f'm_Li_{process_name}', f'E_{process_name}',
                       f'limestone_{process_name}', f'waste_{process_name}'],
...
def proc_MnZn_removal_lime(m_in=m_out) :
    ...
    df_data = {
        'Variables' : [f'm_output_{process_name}', f'm_in_{process_name}', f'E_{process_name}', f'lime_{process_name}',
                       f'water_{process_name}', f'waste_Mn_{process_name}', f'waste_Zn_{process_name}',
                       f'Ca_mass_{process_name}'],
        'Values' : [m_output, m_in, E_MnZn, lime_MnZn, water_lime, waste_Mn, waste_Zn, Ca_mass_brine]
        }
...
def proc_acidification(m_in=m_out) :
    ...
    df_data = {
        'Variables' : [f'm_output_{process_name}', f'm_in_{process_name}', f'HCl_mass32_{process_name}'],
...
def proc_Mg_removal_soda(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"E_Mg_{process_name}",
            f"sodaash_Mg_{process_name}",
            f"water_soda_Mg_{process_name}",
            f"MgCO3_waste_{process_name}",
            f"NaCl_waste_{process_name}",
            ],
        "Values" : [
            m_output,
            m_in,
...
def proc_ion_exchange_L(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"elec_IX_{process_name}",
            f"water_IX_{process_name}",
            f"HCl_IX_{process_name}",
            f"NaOH_IX_{process_name}",
            f"heatwaste_IX_{process_name}",
            f"Cl_IX_{process_name}",
            f"Na_IX_{process_name}",
            ],
        "Values" : [
            m_output,
            m_in,
...
def proc_reverse_osmosis(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"elec_osmosis_{process_name}",
            f"water_RO_{process_name}",
            ],
        "Values" : [
            m_output,
            m_in,
...
def proc_triple_evaporator(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"E_evap_{process_name}",
            f"elec_evap_{process_name}",
            f"steam_{process_name}",
            f"water_evap_{process_name}",
            ],
        "Values" : [
            m_output,
            m_in,
...
def proc_Liprec_BG(mass_CO2, prod_libicarb, m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"waste_heat_sum{process_name}",
            f"E_seclicarb_{process_name}"
            ],
        "Values" : [
            m_output,
            m_in,
...
def proc_washing_TG(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [f"m_output_{function_name}", f"m_in_{function_name}", f"water_deion_{function_name}",
                       f"E_deion_{function_name}"],
...
def proc_washing_BG(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [f"m_output_{process_name}", f"m_in_{process_name}", f"water_deion_{process_name}",
                       f"E_deion_{process_name}"],
        "Values" : [m_output, m_in_wash, water_deion, E_deion],
        }
...
def proc_centrifuge_TG(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"elec_centri_{process_name}",
            f"waste_centri_{process_name}",
...
def proc_rot_dry(m_in=m_out) :
    ...
    df_data = {
        "Variables" : [
            f"m_output_{process_name}",
            f"m_in_{process_name}",
            f"E_dry_{process_name}",
            f"elec_dry_{process_name}",
            ],
        "Values" : [
            m_output,
            m_in,
