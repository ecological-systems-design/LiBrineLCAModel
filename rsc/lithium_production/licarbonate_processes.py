...
m_out =1000 # as an initializer for all functions

# Si & Fe removal by precipitation
def proc_SiFe_removal_limestone(site_parameters, m_in) :
    ...
    m_out = df_SiFe_removal_limestone.iloc[0]['Values']
    return m_out, df_SiFe_removal_limestone


# Mn and Zn removal by precipitation
def proc_MnZn_removal_lime(site_parameters, m_in=m_out) :
...
def proc_acidification(site_parameters, m_in=m_out) :
    ...
    m_out = df_acidification.iloc[0]['Values']

    return m_out, df_acidification


# Li-ion selective adsorption
def proc_Li_adsorption(prod, site_parameters, m_in=m_out) :
...
def proc_CaMg_removal_sodiumhydrox(site_parameters, Ca_mass_leftover, m_in=m_out) :
...
def proc_Mg_removal_soda(site_parameters, m_in=m_out) :
...
def proc_Mg_removal_quick(site_parameters, m_in=m_out) :
...
def proc_ion_exchange_L(site_parameters, m_in=m_out) :
...
def proc_ion_exchang_H(site_parameters, m_in=m_out) :
...
def proc_reverse_osmosis(site_parameters, m_in=m_out) :
...
def proc_triple_evaporator(site_parameters, m_in=m_out) :
...
def proc_Liprec_TG(prod, site_parameters, m_in=m_out) :
...
def proc_dissolution(prod, site_parameters, m_in=m_out) :
...
def proc_Liprec_BG(mass_CO2, prod_libicarb, prod, site_parameters, m_in=m_out) :
...
# Washing Li2CO3 (TG)
def proc_washing_TG(prod, site_parameters, m_in=m_out) :
    T_out = site_parameters['annual_airtemp'] #annual air temperature
    boil_point = site_parameters['boilingpoint_process'] #boiling point
...
# Washing Li2CO3 (BG)
def proc_washing_BG(prod, site_parameters, m_in=m_out) :
    boil_point = site_parameters['boilingpoint_process'] #boiling point
    T_out = site_parameters['annual_airtemp'] #annual air temperature
    water_deion = 2 * prod  # Mass of water required for washing [kg]
    E_deion = (((boil_point - T_out) * hCHH * water_deion) / 10 ** 6) / heat_loss  # Thermal energy demand [MJ]
...
# Centrifuge after TG
def proc_centrifuge_TG(prod, site_parameters, m_in=m_out) :
...
# Centrifuge after BG
def proc_centrifuge_BG(prod, site_parameters, m_in=m_out) :
    Dens_ini = site_parameters['density_brine'] #initial density of brine
    elec_centri = 1.3 * m_in / (Dens_ini * 1000)  # Electricity consumption of centrifuge [kWh]
...
def proc_centrifuge_wash(prod, site_parameters, m_in=m_out) :
    Dens_ini = site_parameters['density_brine'] #initial density of brine
    elec_centri = 1.3 * m_in / (Dens_ini * 1000)  # Electricity consumption of centrifuge [kWh]
    waste_centri = -(m_in - 1.5 * prod) / 1000
    m_output = 1.5 * prod
...
# Belt filter
def proc_belt_filter(prod, site_parameters, m_in=m_out) :
...
# Rotary dryer
def proc_rot_dry(prod, site_parameters, m_in=m_out) :
...
# Water purification
def proc_purification(site_parameters, water=m_out) :
    waste = water * 0.25
...
def loop_functions(eff, Li_conc, location, abbrev_loc) :
    # Extract data
    #TODO initial_site_data should be an input of that function
    initial_site_data = extract_data(location, abbrev_loc, Li_conc=Li_conc)

    #TODO add the following to the initial function to quantify prod_year
    vec_end = initial_site_data[abbrev_loc]['vec_end']
    Li_conc = vec_end[0]
    ...
    m_out, df_SiFe_removal_limestone = proc_SiFe_removal_limestone(site_parameters= initial_site_data[abbrev_loc], m_in=m_pumpbr)
    m_out, df_MnZn_removal_lime, Ca_mass_brine = proc_MnZn_removal_lime(site_parameters= initial_site_data[abbrev_loc], m_in=df_SiFe_removal_limestone.iloc[0, 1])
    m_out, df_acidification = proc_acidification(site_parameters= initial_site_data[abbrev_loc], m_in=df_MnZn_removal_lime.iloc[0, 1])
    m_out, df_adsorption = proc_Li_adsorption(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_acidification.iloc[0, 1])
    m_out, df_CaMg_removal_sodiumhydrox = proc_CaMg_removal_sodiumhydrox(site_parameters= initial_site_data[abbrev_loc], Ca_mass_leftover=Ca_mass_brine,
                                                                         m_in=df_adsorption.iloc[0, 1])
    m_out, df_ion_exchange_L = proc_ion_exchange_L(site_parameters= initial_site_data[abbrev_loc], m_in=df_CaMg_removal_sodiumhydrox.iloc[0, 1])
    m_out, df_reverse_osmosis, water_RO = proc_reverse_osmosis(site_parameters= initial_site_data[abbrev_loc], m_in=df_ion_exchange_L.iloc[0, 1])
    m_out, df_triple_evaporator, water_evap = proc_triple_evaporator(site_parameters= initial_site_data[abbrev_loc], m_in=df_reverse_osmosis.iloc[0, 1])
    m_out, df_Liprec_TG = proc_Liprec_TG(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_triple_evaporator.iloc[0, 1])
    m_out, df_washing_TG = proc_washing_TG(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_Liprec_TG.iloc[0, 1])
    m_out, df_centrifuge_TG = proc_centrifuge_TG(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_washing_TG.iloc[0, 1])
    m_out, df_dissolution, mass_CO2, prod_libicarb = proc_dissolution(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_centrifuge_TG.iloc[0, 1])
    m_out, df_Liprec_BG = proc_Liprec_BG(mass_CO2, prod_libicarb, prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_dissolution.iloc[0, 1])
    m_out, df_centrifuge_BG = proc_centrifuge_BG(prod,site_parameters= initial_site_data[abbrev_loc], m_in=df_Liprec_BG.iloc[0, 1])
    m_out, df_washing_BG = proc_washing_BG(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_centrifuge_BG.iloc[0, 1])
    m_out, df_centrifuge_wash = proc_centrifuge_wash(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_washing_BG.iloc[0, 1])
    m_out, df_rotary = proc_rot_dry(prod, site_parameters= initial_site_data[abbrev_loc], m_in=df_centrifuge_wash.iloc[0, 1])
